...
contract("CDPCreator", accounts => {
  ...
  beforeEach(async () => {
    cdpCreator = await CDPCreator.new(
  ...
  describe("Creation Tests", () => {
    it("Creates a CDP and transfers it to the creator along with drawn DAI", async () => {
      const tx = await cdpCreator.createCDP(dai, {
        value: min,
        from: accounts[0]
      });
    ...
    it("should not create a CDP and draw a lot of dai", async () => {
      await cdpCreator.createCDP(dai * 200, { value: min, from: accounts[0] })
        .should.be.fulfilled;
      await cdpCreator.createCDP(dai * 300, { value: min, from: accounts[0] })
        .should.be.rejected;
    });

    it("should revert with an amount < 0.005 ETH", async () => {
      await cdpCreator.createCDP(0, {
